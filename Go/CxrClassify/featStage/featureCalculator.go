...
func (f *FeatureCalculator) processor(filePath string, connection *sql.DB) {
	// Read file
	...
	txn, err := connection.BeginTx(context.Background(), nil)
	if err != nil {
		log.Println(err)
	}

	_, err = txn.ExecContext(context.Background(), sqlQuery)
	if err != nil {
		log.Println(err)
	}
